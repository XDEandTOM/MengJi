...
- [ ] **笔记搜索高亮** — 搜索关键词在笔记卡片中高亮显示
- [ ] **编辑器支持粘贴图片** — 直接粘贴剪贴板图片自动上传
- [ ] **暗色模式持久化** — 记住用户偏好，页面刷新不丢失

### 🥈 中优先级

- [ ] **修复笔记导出/导入** — 后端补齐 `/api/notes/export` 和 `/api/notes/import` 路由
- [ ] **置顶顺序拖拽调整** — 手动拖拽调整置顶笔记的顺序
- [ ] **Todo List 支持** — Markdown 中 `- [ ]` / `- [x]` 可点击切换
