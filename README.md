...
**碎碎 (SuiSui)** 是一个**单二进制部署**的碎片化笔记应用。支持 Markdown 编辑、图片上传、Emoji 反应、标签分类、活动热力图、多用户权限和后台管理。前端 Vue 3 + 后端 Go 编译为**一个可执行文件**，开箱即用。
...
| 活动热力图 | 月度日历 · 按碎片笔记数量着色 · **点击跳转到当天碎片笔记** |
| 暗色模式 | 一键切换 · 跟随系统 |
| 自定义主题色 | 每位用户独立设置主色 |
| 响应式适配 | 桌面侧边栏 · 移动端底部导航 |
...
- 🎨 工具栏支持水平滚动，移动端不丢失按钮
- 🐛 修复 todo list 左侧内边距过大
- 🐛 修复头像文字在深浅主题切换时不反色

### v1.3.5

- 🎨 **UI 全面美化** — 侧边栏加用户头像和昵称、卡片阴影升级、空状态插画
- ✨ 编辑器工具栏新增 6 个功能按钮（有序列表、删除线、待办、代码块、表格、分隔线）

### v1.3.4

- 🔒 **上线前安全加固** — POST 笔记/上传/反应端点补全 token 校验
- 🔒 修复 6 处 SQL 错误信息泄露到前端
- 🔒 添加安全响应头（X-Frame-Options、Referrer-Policy）
- 🔒 GET /api/admin/stats 加管理员角色检查
- 🐛 修复上传文件/字体从 CDN 加载慢的问题
- 🐛 修复 Docker 持久化卷覆盖二进制文件的问题
- 📦 Docker 镜像自动构建并推送到 Docker Hub
- 🐙 在线预览链接：https://suisui.malaoer.top

### v1.3.3 (2026-06-09)

- ✨ **热力图点击筛选日期** — 点击日历中的某一天，笔记列表立即筛选出当天的碎片笔记，支持与搜索 + 标签联动
