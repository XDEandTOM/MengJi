...
async function onInlineUpload(e: Event) {
  ...
  if (files.some(f => f.size > 10 * 1024 * 1024)) { alert("ͼƬ��С���ܳ��� 10MB"); input.value = ""; return }
    ...
    <div class="main-col">
      ...
      <div v-if="auth.isLoggedIn" class="inline-editor mb-4">
        <div class="editor-box">
          <textarea ref="inlineTextarea" v-model="inlineContent" class="inline-textarea"
            placeholder="д��ʲô��.." rows="1" @keydown="onInlineKeydown" @input="autoGrowTextarea"></textarea>
          ...
          <div class="editor-toolbar">
            ...
            <v-btn color="#1976D2" size="small" variant="flat" class="rounded-pill px-4 submit-btn" @click="submitInline">
              <v-icon start size="x-small">mdi-send</v-icon>{{ editingNoteId ? "����" : "����" }}
            </v-btn>
          </div>
          <v-expand-transition>
            <div v-if="showInlineTags" class="pa-3">
              <v-text-field v-model="inlineTagsInput" label="��ǩ�����ŷָ���" variant="outlined" hide-details density="compact" placeholder="vue, memos, md" />
            </div>
          </v-expand-transition>
        </div>
      </div>
      ...
      <template v-else>
        <div v-if="filteredNotes.length === 0" class="empty-state">
          <p class="empty-text">{{ (searchQuery || selectedTag) ? "û���ҵ�ƥ��ı���¼" : "��û�б���¼" }}</p>
        </div>
