...
const previewUrl = ref(auth.userAvatar || '')
const fileInput = ref<HTMLInputElement | null>(null)

// Track which method was last used for the current session
...
function onTabChange(newTab: 'upload' | 'url') {
  // Clear the opposite input when switching tabs
...
async function save() {
  await auth.updateAvatar(previewUrl.value)
  ...
  <v-dialog :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)"
    ...
    <v-card class="rounded-xl" rounded="xl">
      <v-card-title class="d-flex align-center pa-4 pb-0">
        ...
        <span class="text-h6 font-weight-medium">设置头像</span>
        <v-spacer />
      ...
      <div class="d-flex justify-center pa-4 pb-2">
        <div style="position:relative;display:inline-block">
          ...
          <v-img v-if="isImage(previewUrl)" :src="previewUrl" alt="" cover />
          <span v-else-if="previewUrl" class="text-h4">{{ previewUrl }}</span>
          <span v-else class="text-h5 font-weight-medium">{{ (auth.userNickname || auth.userName).charAt(0).toUpperCase() }}</span>
        </v-avatar>
          <v-chip v-if="activeMethod" size="x-small" variant="flat" color="primary" class="method-badge"
            style="position:absolute;bottom:-4px;right:-4px;font-size:0.65rem">
      ...
      <v-card-text class="pa-4 pt-2">
        ...
        <div v-if="tab === 'upload'" class="text-center">
          ...
          <p v-if="activeMethod === 'url'" class="text-caption text-warning mt-2">切换到该方式将替换链接头像</p>
        </div>
        ...
        <div v-if="tab === 'url'">
          ...
          <div class="d-flex ga-2 mb-3">
            <v-text-field v-model="urlInput" variant="outlined" density="compact" hide-details
              placeholder="https://example.com/avatar.png" class="flex-1" @keyup.enter="useUrl" />
          </div>
          <v-btn variant="tonal" color="primary" class="rounded-pill" block @click="useUrl">预览</v-btn>
          <p v-if="activeMethod === 'upload'" class="text-caption text-warning mt-2">切换到该方式将替换上传的头像</p>
        </div>
      </v-card-text>
      ...
      <v-card-actions class="pa-4 pt-0 d-flex ga-2">
        ...
        <v-btn v-if="previewUrl && previewUrl !== auth.userAvatar"
          variant="text" color="error" class="rounded-pill" @click="removeAvatar">清除</v-btn>
...
.method-badge { position: absolute; bottom: 0; right: 0; font-size: 0.65rem; }
