...
<template>
  <v-dialog :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)" max-width="400">
    <v-card class="rounded-xl pa-4">
      <v-card-title class="text-subtitle-1 font-weight-medium px-0">应用图标</v-card-title>
      <v-card-text class="px-0 d-flex flex-column align-center ga-3">
        <v-img v-if="auth.userAppIcon" :src="auth.userAppIcon" width="80" height="80" class="rounded-lg" />
        <v-btn variant="outlined" color="primary" :loading="uploading" @click="() => $refs.fileInput?.click()">上传图标</v-btn>
        <input ref="fileInput" type="file" accept="image/*" hidden @change="onUpload" />
        <v-btn v-if="auth.userAppIcon" variant="text" color="error" size="small" @click="auth.updateAppIcon('')">清除图标</v-btn>
      </v-card-text>
